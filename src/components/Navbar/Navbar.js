...
const Navbar = () => {
  ...
  const navItem = (
    <>
      ...
      <li className="dropdown dropdown-hover hover:border-none bg-[#453364]" >
        ...
        <ul tabIndex={0} className="w-24 p-2 shadow dropdown-content menu rounded-box lg:w-52 bg-[#453364]"  >
          <li className="p-2 nav-item">
            <NavLink
              className={({ isActive }) => (isActive ? `${active}` : `${normal}`)}
              to="/blog"
  ...
  return (
    <div className="sticky top-0 z-50 header blog">
      ...
      <div className="flex items-center justify-between gap-56 px-4 md:pl-10 lg:px-14 header lg:gap-28 bg-[#453364]" >
        <div className="navbar header">
          <div className="lg:hidden">
            ...
            <div
              className={`fixed top-0 left-0 w-64 h-full bg-gray-00 text-white transform transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
              style={{ transitionDuration: "300ms" }}
            >
              <button onClick={toggleDrawer} className="absolute top-4 right-4">
                <IoClose className="w-6 h-6" />
              </button>
              <ul className="p-4 mt-12">{navItem}</ul>
            </div>
          </div>
        ...
        <div className="navbar-end">
          <a className="bg-green-500 rounded-full btn btn-sm animate-pulse" href="https://wa.me/+8801619848555" title="WHATSAPP CHAT">
            <BsWhatsapp />
          </a>
        </div>
      </div>
